...
% Last Modified by GUIDE v2.5 13-Sep-2022 09:36:10
...
% --- Executes on button press in pushbutton_RT.
function pushbutton_RT_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_RT (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
color = myColor();
set(handles.pushbutton_RT, 'enable', 'off');
set(handles.pushbutton_RT, 'backgroundcolor', color.chinaRed);
set(handles.pushbutton_RT, 'string', 'Runing');
pause(1e-1);
...
try
    if flagRelayTestSim
        ...
        while (abs(sys_out_amplitude-err_out_amplitude) > 1e-2)
            ...
            Tc_ntime = (time_end-time_start) / (period_count_end-period_count_start);
            for nT = 1:1:(period_count_end-period_count_start)
                sys_out_T = sys_out((time_start+Tc_ntime*(nT-1)+1):(time_start+Tc_ntime*nT));
                sys_out_amplitude_T(nT) = (max(sys_out_T) - min(sys_out_T))/2;
                err_out_T = err_out((time_start+Tc_ntime*(nT-1)+1):(time_start+Tc_ntime*nT));
        ...
        Tc = (time(time_end)-time(time_start)) / (period_count_end-period_count_start);
        Kc = abs(((4*h)/(pi*a))*sqrt(1-(d/a)^2) - 1j*(4*h*d)/(pi*a^2));
        % 传递参数
        handles.L = L;
        handles.GpL = GpL;
        handles.Gp  = Gp;
        handles.Ku = Kc;                  % critical gain
        handles.Tu = Tc;                  % critical period
        guidata(hObject, handles)
    else
        Ns = str2num(get(handles.edit_Ns,  'string'));                   % 读取系统
        Ms = str2num(get(handles.edit_Ms,  'string'));
...
set(handles.pushbutton_Tuning, 'enable', 'on');
set(handles.pushbutton_RT, 'backgroundcolor', color.skyBlue);
set(handles.pushbutton_RT, 'string', 'Get');
set(handles.pushbutton_RT, 'enable', 'on');
...
try
    ...
    [Kp, Ti, Td] = Function_ZNPID_mu(handles.mu, Ku, Tu);
    set(handles.edit_Kc, 'string', num2str(Kp,'%.4f'));
    ...
    handles.Td = Td;
    s  = tf('s');
    handles.Gc = Kp*(1 + 1/(Ti*s) + Td*handles.dfo);
    handles.flagRun = true;
    guidata(hObject, handles);
catch
    errordlg('\fontsize{10} Input Error: \lambda, Ku or Tu!', 'ERROR', handles.opts);
end
...
try
    ...
    hold on;
    plot(squeeze(Re), squeeze(Im), 'Color',color.kleinBlue, 'LineWidth',2, 'DisplayName','PID^{\mu}');
    plot(-0.6,-0.6*(pi/4-1/pi),'.', 'Color',color.burgundyRed,'MarkerSize',25, 'LineWidth',2, 'DisplayName','ZN-point');
    axis([-1.2 -0.2 -1.2 0.5]);
    legend;
    grid on;
    hold off
catch
...
plot(time, sys_out, '-', 'Color',color.burgundyRed, 'LineWidth',2, 'DisplayName','PID^\mu');
...
% --- Executes on button press in pushbutton_Rest.
function pushbutton_Rest_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_Rest (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(handles.pushbutton_Rest, 'enable', 'off');
...
guidata(hObject, handles);
clc;
set(handles.pushbutton_Rest, 'enable', 'on');
